...
O arquivo `admin-aulas.html` usa o mesmo login administrativo já configurado (`ADMIN_EMAIL` e `ADMIN_PASSWORD`). Nele, você pode adicionar, editar e excluir modalidades, além de alterar planos, preços, professores e horários. `admin-site.html` permite editar os textos principais, horários de abertura/fechamento da locação, preços de 1h/2h e mensais, ativar/configurar a promoção de sexta e editar a galeria (trocar, adicionar, remover fotos e editar legendas). Os horários também controlam as faixas oferecidas no calendário de reservas. O site público lê as configurações por `/api/classes` e `/api/site-settings`; mudanças administrativas são protegidas por autenticação.

No Render, conecte o repositório GitHub como Web Service apontando a pasta `backend` (Build: `npm install`; Start: `npm start`). Configure um disco persistente e defina `DATA_DIR` para o caminho de montagem (por exemplo, `/var/data`). Esse disco mantém `class-settings.json`, `site-settings.json` e as fotos enviadas à galeria após novos deploys. Não é necessário contratar um banco relacional para esses dados. Sem o disco, os botões de salvar agora mostram um erro e não fingem que as mudanças foram gravadas. Publique os arquivos estáticos da pasta `public_html` na Locaweb. A API continua usando a URL Render já configurada nos arquivos do site.
